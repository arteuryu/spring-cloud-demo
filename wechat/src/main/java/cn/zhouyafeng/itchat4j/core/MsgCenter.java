...
public class MsgCenter {
	...
	public static JSONArray produceMsg(JSONArray msgList) {
		...
		for (int i = 0; i < msgList.size(); i++) {
			...
			LOG.info("收到消息一条，来自: " + m.getString("FromUserName")+";内容:"+m.getString("Content"));
			result.add(m);
		}
		return result;
	}
	...
	public static void handleMsg(IMsgHandlerFace msgHandler) {
		while (true) {
			...
			//					MessageTools.sendMsgById(result, core.getMsgList().get(0).getString("FromUserName"));
								MessageTools.sendFileMsgByUserId(core.getMsgList().get(0).getString("FromUserName"),result);
								...
								MessageTools.sendFileMsgByUserId(core.getMsgList().get(0).getString("FromUserName"),"/Users/yuerte969/Downloads/text2audio.mp3");
					//			MessageTools.sendMsgById(result, core.getMsgList().get(0).getString("FromUserName"));
